/* eslint-disable */
/* REMEMBER to keep the above line to disable the eslint warnings!!!  */
...
(function() {
  ...
  AjaxRequest.prototype = {
    ...
    prepareData: function() {
      if (this.p.method === 'POST' && !this.isFormData())
  ...
  Dom.prototype = {
    ...
    wrap: function(html) {
      return this._inject(html, function(frag, node) {
        var wrapper =
          typeof frag === 'string'
            ? this.create(frag)[0]
            : frag instanceof Node ? frag : this._toArray(frag)[0];
    ...
    _adjustResultHeightOrWidth: function(type, el, adjust) {
      ...
      if (type === 'height') {
        ...
        if (adjust === 'outer')
          fix -= (parseFloat(style.marginTop) || 0) + (parseFloat(style.marginBottom) || 0);
      } else {
        if (adjust === 'inner' || (adjust === 'normal' && isBorderBox)) {
          fix +=
        ...
        if (adjust === 'outer')
          fix -= (parseFloat(style.marginLeft) || 0) + (parseFloat(style.marginRight) || 0);
      }
  ...
  var RedactorApp = function(selector, options, args) {
    ...
    var nodes = Array.isArray(selector)
      ? selector
      : selector && selector.nodeType ? [selector] : document.querySelectorAll(selector);
    ...
    return value.length === 0 || value.length === 1
      ? value.length === 0 ? instance : value[0]
      : value;
  };
        ...
        $elements.each(
          function(node) {
            ...
            for (var key in params) {
              ...
              oldVal = this.utils.isRgb(oldVal)
                ? this.utils.rgb2hex(oldVal)
                : oldVal.replace(/"/g, '');
              newVal = this.utils.isRgb(newVal)
                ? this.utils.rgb2hex(newVal)
                : newVal.replace(/"/g, '');
    ...
    set: function(duration, iterate, timing) {
      ...
      while (len--) {
        if (duration !== false || duration === '')
          this.$el.css(this.prefixes[len] + 'animation-duration', duration);
        if (iterate !== false || iterate === '')
          this.$el.css(this.prefixes[len] + 'animation-iteration-count', iterate);
        if (timing !== false || timing === '')
          this.$el.css(this.prefixes[len] + 'animation-timing-function', timing);
      }
    },
    clean: function() {
  ...
  $R.add('service', 'caret', {
    ...
    setAtPrev: function(node) {
      ...
      if (prev) {
        prev =
          prev.nodeType === 3 && this._isEmptyTextNode(prev) ? prev.previousElementSibling : prev;
  ...
  $R.add('service', 'selection', {
    ...
    isText: function() {
      ...
      if (sel) {
        ...
        if (
          (block && this.inspector.isTableCellTag(block.tagName)) ||
          (block === false && blocks.length === 0)
        ) {
          return true;
        }
      }
    ...
    isAll: function(el) {
      ...
      if (
        !data.isFigcaption() &&
        this.component.isNonEditable(node) &&
        this.component.isActive(node)
      ) {
        return true;
      }
    ...
    getInlinesAllSelected: function(options) {
      ...
      finalNodes =
        options && options.tags ? this._filterNodesByTags(finalNodes, options.tags) : finalNodes;
    ...
    getInlines: function(options) {
      ...
      filteredNodes =
        options && options.tags
          ? this._filterNodesByTags(filteredNodes, options.tags)
          : filteredNodes;
      filteredNodes =
        options && options.inside
          ? this._filterInlinesInside(filteredNodes, options)
          : filteredNodes;
    ...
    getBlocks: function(options) {
      ...
      filteredNodes =
        options && options.tags
          ...
          : filteredNodes;
      filteredNodes =
        options && options.first ? this._filterBlocksFirst(filteredNodes, options) : filteredNodes;
  ...
  $R.add('class', 'toolbar.dropdown', {
    ...
    isActive: function() {
      var $dropdown = this.$body.find(
        '#redactor-dropdown-' + this.uuid + '-' + this.name + '.open'
      );
    ...
    open: function(e) {
      ...
      this.$doc.on(
        'click.redactor.dropdown-' + this.uuid + ' touchstart.redactor.dropdown-' + this.uuid,
  ...
  $R.add('service', 'cleaner', {
    ...
    paste: function(html) {
      ...
      // remove style
      var filterStyle =
        this.opts.pasteKeepStyle.length !== 0 ? ',' + this.opts.pasteKeepStyle.join(',') : '';
      $elms.not('[data-redactor-style-cache]' + filterStyle).removeAttr('style');

      // remove class
      var filterClass =
        this.opts.pasteKeepClass.length !== 0 ? ',' + this.opts.pasteKeepClass.join(',') : '';
      $elms.not('[data-redactor-style-cache]' + filterClass).removeAttr('class');

      // remove attrs
      var filterAttrs =
        this.opts.pasteKeepAttrs.length !== 0 ? ',' + this.opts.pasteKeepAttrs.join(',') : '';
    ...
    encodePreCode: function(html) {
      var matched = html.match(
        new RegExp('<code(.*?)>(.*?)<pre(.*?)>(.*?)</pre>(.*?)</code>', 'gi')
      );
      if (matched !== null) {
    ...
    _cleanWrapped: function(html) {
      html = html.replace(
        new RegExp('<p><figure([\\w\\W]*?)</figure></p>', 'gi'),
        '<figure$1</figure>'
      );
    ...
    _encodePreCodeLine: function(node) {
      ...
      encoded = this.opts.preSpaces
        ? encoded.replace(/\t/g, new Array(this.opts.preSpaces + 1).join(' '))
        : encoded;
  ...
  $R.add('class', 'cleaner.figure', {
    ...
    unconvert: function(html, rules) {
      ...
      $wrapper
        .find('figure[data-redactor-type=image]')
        .removeAttr('rel')
        .each(this._unconvertImages.bind(this));

      $wrapper
        .find('img')
        .removeAttr('data-redactor-type')
        .removeClass('redactor-component');
  ...
  $R.add('class', 'cleaner.paragraphize', {
    ...
    _convert: function(html) {
      ...
      // clean restored
      html = this.opts.breakline
        ? html
        : html.replace(
            new RegExp('<' + markupTag + '><br\\s?/?></' + markupTag + '>', 'gi'),
            '<' + markupTag + '></' + markupTag + '>'
          );
  ...
  $R.add('class', 'inspector.parser', {
    ...
    getComponentCodeElement: function() {
      return this.isComponentType('code')
        ? this.$component
            .find('pre code, pre')
            .last()
            .get()
  ...
  $R.add('service', 'insertion', {
    ...
    insertHtml: function(html, clean) {
      ...
      if (this.editor.isEmpty()) {
        ...
        parsedInput.html =
          clean !== false
            ? this.cleaner.removeTagsExcept(parsedInput.html, ['a'])
            : parsedInput.html;
        parsedInput.html =
          clean !== false ? this.cleaner.replaceNbspToSpaces(parsedInput.html) : parsedInput.html;
        ...
        if (!isText && this.opts.markup !== 'br' && this._hasBlocksAndImages(parsedInput.nodes)) {
          parsedInput.html =
            clean !== false ? this.cleaner.paragraphize(parsedInput.html) : parsedInput.html;
        ...
        parsedInput.html =
          clean !== false ? this.cleaner.replaceBlocksToBr(parsedInput.html) : parsedInput.html;
        parsedInput.html =
          clean !== false
            ? this.cleaner.removeTagsExcept(parsedInput.html, except)
            : parsedInput.html;
        ...
        parsedInput.html =
          clean !== false
            ...
            : parsedInput.html;
        parsedInput.html =
          clean !== false ? this.cleaner.removeBrAtEnd(parsedInput.html) : parsedInput.html;
    ...
    _insertToCode: function(parsedInput, current, clean) {
      parsedInput.html =
        clean !== false ? this.cleaner.encodeHtml(parsedInput.html) : parsedInput.html;
      parsedInput.html =
        clean !== false ? this.cleaner.removeNl(parsedInput.html) : parsedInput.html;
    ...
    _sendNodes: function(nodes) {
      for (var i = 0; i < nodes.length; i++) {
        var el = nodes[i];
        var type =
          el.nodeType !== 3 && typeof el.getAttribute === 'function'
            ? el.getAttribute('data-redactor-type')
            : false;
        if (type) {
    ...
    _getCleanedInput: function(parsedInput, dataCurrent, clean) {
      ...
      parsedInput.html =
        !isPreformatted && clean !== false
          ? this.cleaner.input(parsedInput.html)
          : parsedInput.html;
      parsedInput =
        !isPreformatted && clean !== false ? this.utils.parseHtml(parsedInput.html) : parsedInput;
    ...
    _getTextNodesData: function() {
      ...
      while (!stop) {
        ...
        if (!node) {
          ...
          node.nodeType !== 3 &&
          (node.tagName === 'BR' || this.inspector.isBlockTag(node.tagName))
        ) {
          stop = true;
        } else {
          nodes.push(node);
    ...
    _cleanBlocks: function(nodes) {
      ...
      for (var i = 0; i < nodes.length; i++) {
        ...
        if (headings.indexOf(tag) !== -1) {
          $node
            .find('span')
            .not('.redactor-component, .non-editable, .redactor-selection-marker')
            .unwrap();
        } else if (tag === 'pre') {
          $node
            .find(tags.join(','))
  ...
  $R.add('service', 'inline', {
    ...
    _isFormat: function() {
      ...
      var data = this.inspector.parse(current);
      var isComponent =
        data.isComponent() && !data.isComponentType('table') && !data.isFigcaption();
    ...
    _revertToInlines: function() {
      ...
      // span convertable
      $editor.find('span.redactor-convertable-apply').each(
        function(node) {
          var $node = $R.dom(node);
    ...
    getInlines: function(tags) {
      return tags
        ? this.selection.getInlines({ tags: tags, all: true })
        : this.selection.getInlines({ all: true });
    },
    getElements: function(tags) {
  ...
  $R.add('service', 'autoparser', {
    ...
    parse: function(html) {
      ...
      for (var i = 0; i < tags.length; i++) {
        var reTags =
          singleReplace.indexOf(tags[i]) !== -1
            ? '<' + tags[i] + '[^>]*>'
            : '<' + tags[i] + '([\\w\\W]*?)</' + tags[i] + '>';
      ...
      if (
        ...
        (html.match(this.opts.regex.youtube) || html.match(this.opts.regex.vimeo))
      ) {
        var iframeStart =
          '<iframe width="500" height="281" class="redactor-autoparser-object" src="';
    ...
    _format: function() {
      ...
      var isNotFormatted =
        parent && $parent.closest('figure, pre, code, img, a, iframe').length !== 0;
      ...
      if (current && current.nodeType === 3) {
        ...
        // images
        if (
          this.opts.autoparseImages &&
          content.match(this._convertToRegExp(this.opts.regex.imageurl))
        ) {
          ...
          content.match(this._convertToRegExp(this.opts.regex.url))
        ) {
          // links
    ...
    _formatLinks: function(content) {
      ...
      for (var i = 0; i < matches.length; i++) {
        ...
        var target =
          this.opts.pasteLinkTarget !== false ? ' target="' + this.opts.pasteLinkTarget + '"' : '';

        text =
          text.length > this.opts.linkSize ? text.substring(0, this.opts.linkSize) + '...' : text;
        text = text.search('%') === -1 ? decodeURIComponent(text) : text;

        // escaping url
        var regexp =
          '(' + href.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, '\\$&') + regexB + ')';
  ...
  $R.add('service', 'utils', {
    ...
    isEmpty: function(el) {
      ...
      if (el) {
        isEmpty =
          el.nodeType === 3 ? el.textContent.trim().replace(/\n/, '') === '' : el.innerHTML === '';
      }
    ...
    splitNode: function(current, nodes, isList, inline) {
      ...
      var element;
      if (inline) {
        element = this.inspector.isInlineTag(current.tagName)
          ? current
          : this.selection.getInline(current);
      } else {
        element = this.inspector.isBlockTag(current.tagName)
          ? current
          : this.selection.getBlock(current);
      }
    ...
    extractHtmlFromCaret: function(inline, element) {
      ...
      if (range) {
        element = element
          ? element
          : inline ? this.selection.getInline() : this.selection.getBlock();
        if (element) {
          var clonedRange = range.cloneRange();
    ...
    checkProperty: function(obj) {
      var args =
        arguments[1] && Array.isArray(arguments[1]) ? arguments[1] : [].slice.call(arguments, 1);
  ...
  $R.add('class', 'editor.placeholder', {
    ...
    build: function() {
      ...
      if (this.opts.placeholder !== false || $element.attr('placeholder')) {
        var text =
          this.opts.placeholder !== false ? this.opts.placeholder : $element.attr('placeholder');
  ...
  $R.add('module', 'source', {
    ...
    _onTabKey: function(e) {
      ...
      $source.val(
        $source.val().substring(0, start) + '    ' + $source.val().substring(el.selectionEnd)
      );
  ...
  $R.add('class', 'source.selection', {
    ...
    insertMarkersToSource: function(html) {
      ...
      if (startOffset === sizeOffset) {
        ...
        html =
          html.substr(0, endOffset + markerLength) +
          markerEnd +
          html.substr(endOffset + markerLength);
      } else {
        this.markersOffset = false;
      }
  ...
  $R.add('module', 'clicktoedit', {
    ...
    _build: function() {
      ...
      $editor.on(
        'click.redactor-click-to-edit mouseup.redactor-click-to-edit',
  ...
  $R.add('module', 'contextbar', {
    ...
    _buildPosition: function(e, $el, position) {
      ...
      var docScrollTop = isTarget
        ? this.$target.scrollTop() + this.$doc.scrollTop()
        : this.$doc.scrollTop();
  ...
  $R.add('class', 'toolbar.air', {
    ...
    _close: function(e) {
      var $el = e ? $R.dom(e.target) : false;
      var isDropdownCall =
        e && $el.closest('[data-dropdown], .redactor-dropdown-not-close').length !== 0;
  ...
  $R.add('class', 'toolbar.fixed', {
    ...
    _doFixed: function() {
      ...
      var boxEnd =
        boxOffset + $container.height() - (toleranceEnd + this.opts.toolbarFixedTopOffset);
      var scrollOffset = this.$fixedTarget.scrollTop();
      var top = !this.toolbar.isTarget()
        ? 0
        : this.$fixedTarget.offset().top - this.$win.scrollTop();

      if (scrollOffset > boxOffset && scrollOffset < boxEnd) {
        var position = this.detector.isDesktop() ? 'fixed' : 'absolute';
        top = this.detector.isDesktop()
          ? top
  ...
  $R.add('module', 'link', {
    ...
    _insertSingle: function(links, data) {
      ...
      if (
        (links.length === 1 && links[0].textContext === this.selection.getText()) ||
        (inline && inline.tagName === 'A')
      ) {
  ...
  $R.add('module', 'input', {
    ...
    _isArrowKey: function(key) {
      return (
        [this.keycodes.UP, this.keycodes.DOWN, this.keycodes.RIGHT, this.keycodes.LEFT].indexOf(
          key
        ) !== -1
      );
    },
  ...
  $R.add('class', 'input.arrow', {
    ...
    _removeInvisibleSpace: function() {
      var current = this.selection.getCurrent();
      var isEmpty = current
        ? this.utils.isEmptyHtml(current.nodeType === 3 ? current.textContent : current.innerHTML)
        : false;
      if (
        isEmpty &&
        current &&
        current.nodeType === 3 &&
        this.utils.searchInvisibleChars(current.textContent) === 0
      ) {
  ...
  $R.add('class', 'input.delete', {
    ...
    _traverseDelete: function(e) {
      ...
      if (isEnd && dataNext.isComponentEditable()) {
        ...
        !data.isTable() &&
        isNextBlock &&
        !this.utils.isEmptyHtml(block.innerHTML)
      ) {
        // block
        e.preventDefault();
      ...
      if (isStart && dataPrev.isComponentEditable()) {
        ...
        if (dataPrev.isList()) {
          $currentList
            .children('li')
            .first()
            .prepend(this.marker.build('start'));
    ...
    _removeInvisibleSpace: function() {
      ...
      if (
        this.key === this.keycodes.DELETE &&
        prev &&
        prev.nodeType === 3 &&
        this.utils.searchInvisibleChars(prev.textContent) === 0
      ) {
      ...
      setTimeout(function() {
        $editor.find('pre').each(function(node) {
          ...
          if ($node.closest('figure').length === 0) {
            $node
              .find(tags.join(','))
              .not('code, .redactor-selection-marker')
              .unwrap();
          }
        });
      }, 1);
      ...
      if (typeof this.opts.imagePosition === 'object') {
        ...
        switch (align) {
          case 'left':
            imageFloat = 'left';
            imageMargin =
              '0 ' + this.opts.imageFloatMargin + ' ' + this.opts.imageFloatMargin + ' 0';
            break;
          case 'right':
            imageFloat = 'right';
  ...
  $R.add('module', 'list', {
    ...
    toggle: function(type) {
      ...
      var $list = $R
        .dom(block)
        .parents('ul, ol', '.redactor-in')
        .last();
      if (nodes.length === 0 && $list.length !== 0) {
        nodes = [$list.get()];
      }

      nodes = this._isUnformat(type, nodes)
        ? this._unformat(type, nodes)
        : this._format(type, nodes);
